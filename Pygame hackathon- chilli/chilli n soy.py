...
def move(p):
    ...
    if keys[K_LEFT] and p[X]>200:
       v[X] = -14#5
       p[ROW]=1
    elif keys[K_RIGHT] and p[X]<8300:
        v[X] = 14#5
        p[ROW]=0
    elif keys[K_UP]:
        p[ROW]=2
        ...
        v[X] = 0
        p[COL]=0 #"idle" (standing) position
        p[COL]=p[COL]-0.2 #below this line of code we are adding 0.2
               
    p[COL]=p[COL]+0.2 #advancing to the "next" frame
    if p[COL]>=6:#len(p[ROW]):
        p[COL]=1
def pog():
    global transition
    screen.fill((0,0,0))
...
Bback=image.load("Masking/colorJupiterr.bmp")
...
Cback2=image.load("Masking/colorJupiterr.bmp").convert()
Dback=image.load("Masking/colorMars.bmp").convert()
...
def L2():
    ...
    if Bback.get_at((HOTOIL[0],HOTOIL[1]))==wallA2 \
       or Bback.get_at((HOTOIL[0]+120,HOTOIL[1]))==wallA2\
       or Bback.get_at((HOTOIL[0]+120,HOTOIL[1]+100))==wallA2\
       or Bback.get_at((HOTOIL[0],HOTOIL[1]+100))==wallA2\
       or Bback.get_at((HOTOIL[0],HOTOIL[1]+100))==wallA2\
       or Bback.get_at((HOTOIL[0]+70,HOTOIL[1]+100))==wallA2\
       or Bback.get_at((HOTOIL[0]+120,HOTOIL[1]+100))==wallA2:
        transition="level3"
        lives=3
        HOTOIL[0]=210
...
def L3():
    ...
    jump1(HOTOIL)
    check(HOTOIL,rocks3)
    if Dback.get_at((HOTOIL[0],HOTOIL[1]))==wallA3 \
       or Dback.get_at((HOTOIL[0]+140,HOTOIL[1]))==wallA3\
       or Dback.get_at((HOTOIL[0]+140,HOTOIL[1]+100))==wallA3\
       or Dback.get_at((HOTOIL[0],HOTOIL[1]+100))==wallA3\
       or Dback.get_at((HOTOIL[0],HOTOIL[1]+137))==wallA3\
       or Dback.get_at((HOTOIL[0]+70,HOTOIL[1]+137))==wallA3\
       or Dback.get_at((HOTOIL[0]+120,HOTOIL[1]+137))==wallA3:
        transition="credits"
        HOTOIL[0]=210
        HOTOIL[1]=450
        mixer.music.load(m1[1])
        mixer.music.play(-1)
n=image.load("story/outro.png")
n=transform.scale(n,(1280,720))
def creditss():
    screen.blit(n,(0,0))
    print("THE END")
...
def colourCollision2(p):
    ...
    if Bback.get_at((p[0],p[1]))==wallS2 \
       or Bback.get_at((p[0]+120,p[1]))==wallS2\
       or Bback.get_at((p[0]+120,p[1]+100))==wallS2\
       or Bback.get_at((p[0],p[1]+100))==wallS2\
       or Bback.get_at((p[0],p[1]+100))==wallS2\
       or Bback.get_at((p[0]+70,p[1]+100))==wallS2\
       or Bback.get_at((p[0]+120,p[1]+100))==wallS2:
        print("collision")
        lives-=1
        p[0]=110
...
def colourCollision3(p):
    ...
    if Bback.get_at((p[0],p[1]))==wallS3 \
       or Bback.get_at((p[0]+120,p[1]))==wallS3\
       or Bback.get_at((p[0]+120,p[1]+100))==wallS3\
       or Bback.get_at((p[0],p[1]+100))==wallS3\
       or Bback.get_at((p[0],p[1]+100))==wallS3\
       or Bback.get_at((p[0]+70,p[1]+100))==wallS3\
       or Bback.get_at((p[0]+120,p[1]+100))==wallS3:
        print("collision")
        lives-=1
        p[0]=210
...
steping2 = (Rect(0, 560, 630, 260), 
             Rect(750, 540, 200, 230),
             Rect(1170, 560, 190, 230),
...
rocks3 = (Rect(0, 665, 350, 100),# first plat
              Rect(330, 665, 1200,  100 ),
              Rect(1930, 680, 90, 50),
              Rect(2050, 555, 9000, 50))
offset=0
...
def drawScene2(screen,p):
    ...
    screen.blit(rocket, (offset+8300,20))
...
#-------------------------------------------------------------------Transitions-------------------------------------------------------
    ...
    if lives<=0:
         ...
         pog()
         transition="pog"
    elif lives==1 and transition !="Menu" and transition !="story"and transition !="story2"and transition !="story3" and transition !="decide"and transition !="intro"and transition !="credits":
        screen.blit(life[1],(427,10))
    elif lives==2 and transition !="Menu" and transition !="story"and transition !="story2"and transition !="story3" and transition !="decide"and transition !="intro"and transition !="credits" :
        screen.blit(life[2],(427,10))
    elif lives==3 and transition !="Menu" and transition !="story"and transition !="story2"and transition !="story3" and transition !="decide"and transition !="intro"and transition !="credits":
